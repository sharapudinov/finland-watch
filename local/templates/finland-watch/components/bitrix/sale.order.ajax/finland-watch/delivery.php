...
if (!empty($arResult["DELIVERY"])) {
    ...
    <div class="title-head-login">
        <p><?= GetMessage("SOA_TEMPL_DELIVERY") ?></p>
    </div>
        ...
        if ($delivery_id !== 0 && intval($delivery_id) <= 0) {
            foreach ($arDelivery["PROFILES"] as $profile_id => $arProfile) {
                ...
                <p class="shipping radio">
                    ...
                    <span class="gray">Стоимость 1000 руб<br/>Смотрите условия доставки</span>
                </p>
                <?
                ...
                <br/><span class="gray"><!-- click on this should not cause form submit -->
                    <?
                    if ($arProfile["CHECKED"] == "Y" && doubleval($arResult["DELIVERY_PRICE"]) > 0):
                        ?>
                        <?= GetMessage("SALE_DELIV_PRICE") ?>
                        :&nbsp;<?= $arResult["DELIVERY_PRICE_FORMATED"] ?><br>
                        <?
                        if ((isset($arResult["PACKS_COUNT"]) && $arResult["PACKS_COUNT"]) > 1):
                            echo GetMessage('SALE_PACKS_COUNT') . '' . $arResult["PACKS_COUNT"] . '<br>';
                        endif;
                ...
                <br/><span class="gray">
                    ...
                    if (count($arDelivery["STORE"]) > 0):
                        ?>
												...
												<br/><?= GetMessage('SOA_ORDER_GIVE_TITLE'); ?>
                                                    :
												<br/><?= htmlspecialcharsbx($arResult["STORE_LIST"][$arResult["BUYER_STORE"]]["TITLE"]) ?><
                    <?
                    endif;
                    ?>
            </span>
